...
export class SixDegreesPathfinder {
  ...
  private async getConnections(nodeId: number, nodeType: 'player' | 'manager'): Promise<{
    ...
    if (nodeType === 'player') {
      ...
      // 1. CLUB TEAMMATES - Find teammates (players who played at the same team)
      ...
      // 2. NATIONAL TEAM CONNECTIONS - Find players with same nationality
      if (player.nationality) {
        const nationalTeammates = await prisma.player.findMany({
          ...
          take: 100 // Reasonable limit for national team
        })
      ...
      // 3. MANAGERS - Find managers who coached this player
